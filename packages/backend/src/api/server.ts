...
import { createMboxImportRouter } from './routes/mbox-import.routes';
...
export async function createServer(modules: ArchiverModule[] = []): Promise<Express> {
	...
	const mboxImportRouter = createMboxImportRouter(authService);
	...
	app.use(`/${config.api.version}/import/mbox`, mboxImportRouter);
